...
impl<T: ToNetworkOrder<W>, W: Write> ToNetworkOrder<W> for Option<T> {
    /// # Example
    /// ```
...
impl<'a, T: FromNetworkOrder<'a, R>, R: Read> FromNetworkOrder<'a, R> for Option<T> {
    /// # Example    
    /// ```
    /// use std::io::Cursor;
...
impl<T: ToNetworkOrder<W>, W: Write, const N: usize> ToNetworkOrder<W> for [T; N] {
    /// # Example    
    /// ```
    /// use type2network::ToNetworkOrder;
    ///
...
impl<'a, T: FromNetworkOrder<'a, R>, R: Read, const N: usize> FromNetworkOrder<'a, R> for [T; N] {
    /// # Example       
    /// ```
    /// use std::io::Cursor;
    /// use type2network::FromNetworkOrder;
...
impl<T, W: Write> ToNetworkOrder<W> for Vec<T>
where
    T: ToNetworkOrder<W>,
{
    /// # Example    
...
impl<'a, T, R: Read> FromNetworkOrder<'a, R> for Vec<T>
where
    T: Default + FromNetworkOrder<'a, R>,
{
    ...
    fn deserialize_from(&mut self, buffer: &mut R) -> std::io::Result<()> {
        ...
        // the vector should be defined with the with_capacity() method to be usable with this
...
impl<T, W: Write> ToNetworkOrder<W> for Box<T>
where
    T: ToNetworkOrder<W>,
{
    /// # Example    
...
impl<'a, T, R: Read> FromNetworkOrder<'a, R> for Box<T>
where
    T: FromNetworkOrder<'a, R>,
{
    /// # Example    
    /// ```
    /// use std::io::Cursor;
...
// impl<W: Write> ToNetworkOrder<W> for Box<dyn ToNetworkOrder> {
//     /// # Example
//     /// ```
//     /// use type2network::ToNetworkOrder;
//     ///
...
// impl<'a, R:Read> FromNetworkOrder<'a, R> forBox<dyn FromNetworkOrder<'a>> {
//     /// # Example
//     /// ```
//     /// use std::io::Cursor;
//     /// use std::ops::Deref;
...
impl<T, W: Write> ToNetworkOrder<W> for PhantomData<T> {
    fn serialize_to(&self, _: &mut W) -> std::io::Result<usize> {
        Ok(0)
    }
}

impl<'a, T, R: Read> FromNetworkOrder<'a, R> for PhantomData<T> {
